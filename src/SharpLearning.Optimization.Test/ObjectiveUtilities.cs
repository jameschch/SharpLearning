...
namespace SharpLearning.Optimization.Test
{
    public static class ObjectiveUtilities
    {
        ...
        public static Task<OptimizerResult> Minimize(double[] x)
        {
            return Task.FromResult(new OptimizerResult(x, Math.Sin(x[0]) * Math.Cos(x[1]) * (1.0 / (Math.Abs(x[2]) + 1))));
        }

        public static Task<OptimizerResult> MinimizeWeightFromHeight(double[] parameters)
        {
            ...
            return Task.FromResult(new OptimizerResult(parameters, cost));
        }
